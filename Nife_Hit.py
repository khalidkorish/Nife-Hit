...
def draw_knife(H):
    global r, c, c_k_anim, knife_out, circle_trans,s
    ...
    if H.crash:  # if the knife crashed
        ...
        if (H.top + H.tran < (circle_trans - r)) & H.k_c_crash:
            ...
            H.top = circle_trans - r+.05 # H.top+0.9 -----> -0.7+0.9 ----> 0.2
            H.bottom = 0.05  # H.bottom+0.9 ------> -0.9+0.9--->0
            H.tran = 0
            H.k_c_crash = False
...
def drawText(string, x, y, choose_view):
    ...
    if choose_view == 1:  # for score
        ...
        glScale(0.0006, 0.0006, 1) # talk to the user
...
def moving():
    ...
    if not knife_out:  # if the knife didn't crash
        ...
        if hit_cntr == N_knife:
            ...
            if t_delay > 7:
                ...
                contin ="press \" r \" to cotinue "
                drawText(contin, -.9, -.9, 3)

                if step == 10:
                    winner = "   You win. go to the"
                    drawText(winner, -.9, .63, 2)
                    winner = "  next level"
                    drawText(winner, -.5, .48, 2)
                else:
    ...
    # if the knife crashed
        ...
        try_a = "you l o s e press \" r \" to :"
        drawText(try_a, -.9, .9, 3)
